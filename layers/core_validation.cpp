    ...
    if (cb_node->push_constant_data_ranges) {
        if (pipeline_layout->push_constant_ranges != cb_node->push_constant_data_ranges) {
            ...
            for (const auto &stage : pPipe->stage_state) {
                ...
                int ret =
                    ValidatePushConstantSetUpdate(it->second, entrypoint->push_constant_used_in_shader, struct_index_hierarchy);
                if (ret != 0) {
                    ...
                    if (ret == 2) {
                        ...
                        objlist.add(pipeline_layout->layout);
                        result |=
                            LogError(objlist, vuid.push_constants_set, "%s in %s of %s is not updated.",
                                     struct_index_hierarchy_string.c_str(), string_VkShaderStageFlags(stage.stage_flag).c_str(),

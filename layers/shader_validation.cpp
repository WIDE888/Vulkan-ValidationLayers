...
void DefineStructMember(const SHADER_MODULE_STATE &src, const spirv_inst_iter &it,
    ...
    for (const auto offset : offsets) {
        local_offset = offset;
        data1 = {};
        data1.offset = data.offset + local_offset;  // data offset is world offset.
        ...
        data.struct_members.emplace_back(std::pair<bool, shader_struct_member>(false, data1));
        ++i;
    }
...
// array_offset is a local offset, not a global offset. A struct's offset is 0.
// An array's offset is considered by its index and size.
void SetUsedBytes(std::vector<uint32_t> array_offsets, shader_struct_member &data) {
    int array_offsets_size = static_cast<int>(array_offsets.size());
    uint32_t block_memory_size = data.size;
    for (uint32_t i = static_cast<int>(array_offsets_size); i < data.array_length_hierarchy.size(); ++i) {
        block_memory_size *= data.array_length_hierarchy[i];
    }

    uint32_t offset = 0;
    if (array_offsets_size) {
        uint32_t block = 1;
        for (int i = static_cast<int>(data.array_length_hierarchy.size() - 1); i >= 0; --i) {
            if (i < array_offsets_size && array_offsets[i] > 0) {
                offset += (block * array_offsets[i]);
            }
            block *= data.array_length_hierarchy[i];
        }
        offset *= data.size;
    }

    uint32_t end = offset + block_memory_size;
    if (data.used_bytes.size() < end) {
        data.used_bytes.resize(end, 0);
    }
    std::memset(data.used_bytes.data() + offset, true, static_cast<std::size_t>(block_memory_size));
}

void RunUsedArray(const SHADER_MODULE_STATE &src, std::vector<uint32_t> array_offsets, uint32_t access_chain_word_index,
                  spirv_inst_iter &access_chain_it, shader_struct_member *data) {
    if (access_chain_word_index < access_chain_it.len()) {
        if (data->array_length_hierarchy.size() > array_offsets.size()) {
            ...
            if (def_it != src.end() && def_it.opcode() == spv::OpConstant) {
                array_offsets.emplace_back(def_it.word(3));
                RunUsedArray(src, array_offsets, access_chain_word_index, access_chain_it, data);
            } else {
                // spv::OpLoad
                // If the index of array is not a const, is a variable, treat it as all array is used.
                if (access_chain_word_index < access_chain_it.len()) {
                    for (uint32_t i = 0; i < data->array_length_hierarchy[0]; ++i) {
                        auto offsets = array_offsets;
                        offsets.emplace_back(i);
                        RunUsedArray(src, offsets, access_chain_word_index, access_chain_it, data);
                    }
                } else {
                    SetUsedBytes(array_offsets, *data);
                }
            }
        } else {
            RunUsedStrcut(src, access_chain_word_index, access_chain_it, data);
        }
    } else {
        SetUsedBytes(array_offsets, *data);
    }
...
void RunUsedStrcut(const SHADER_MODULE_STATE &src, uint32_t access_chain_word_index, spirv_inst_iter &access_chain_it,
    ...
    std::vector<uint32_t> array_offsets_emptry;

    if (access_chain_word_index < access_chain_it.len()) {
        auto strcut_member_index = GetConstantValue(&src, access_chain_it.word(access_chain_word_index));
        ++access_chain_word_index;

        data->struct_members[strcut_member_index].first = true;
        data = &(data->struct_members[strcut_member_index].second);
        RunUsedArray(src, array_offsets_emptry, access_chain_word_index, access_chain_it, data);
    }
}

void SetUsedStructMember(const SHADER_MODULE_STATE &src, const uint32_t variable_id,
                         const std::vector<function_set> &function_set_list, shader_struct_member &data) {
    for (const auto &func_set : function_set_list) {
        ...
        for (auto it = range.first; it != range.second; ++it) {
            ...
            if (access_chain.word(3) == variable_id) {
                RunUsedStrcut(src, 4, access_chain, &data);
            }
        }
    }
}
...
int CoreChecks::ValidatePushConstantSetUpdate(const std::vector<int8_t> &push_constant_data_update,
                                              ...
                                              std::vector<uint32_t> &out_struct_index_hierarchy) const {
    // If size > 0, it means not a struct. A scalar, vec, or mat.
    if (push_constant_used_in_shader.used_bytes.size() > 0) {
        uint32_t offset = push_constant_used_in_shader.offset;
        const auto size = push_constant_data_update.size();
        for (const auto used : push_constant_used_in_shader.used_bytes) {
            if (used) {
                if (offset >= size || push_constant_data_update[offset] == -1) {
                    out_struct_index_hierarchy.emplace_back(offset);
                    return 1;  // not set
                } else if (push_constant_data_update[offset] == 0) {
                    out_struct_index_hierarchy.emplace_back(offset);
                    return 2;  // not update
                }
            }
        ...
        uint32_t index = 0;
        for (const auto struct_member : push_constant_used_in_shader.struct_members) {
            if (struct_member.first) {
                int ret =
                    ValidatePushConstantSetUpdate(push_constant_data_update, struct_member.second, out_struct_index_hierarchy);
                if (ret != 0) {
                    out_struct_index_hierarchy.emplace_back(index);
                    return ret;
                }
            }
            ++index;
...
bool CoreChecks::ValidatePushConstantUsage(const PIPELINE_STATE &pipeline, SHADER_MODULE_STATE const *src,
    ...
    for (auto const &range : *push_constant_ranges) {
        if (range.stageFlags & pStage->stage) {
            found_stage = true;
            std::vector<uint32_t> struct_index_hierarchy;
            ...
            int ret = ValidatePushConstantSetUpdate(push_constant_bytes_set, entrypoint->push_constant_used_in_shader,
                                                    struct_index_hierarchy);
            if (ret != 0) {
                std::string struct_index_hierarchy_string = "Push-constant buffer";

                for (int32_t i = static_cast<int32_t>(struct_index_hierarchy.size()) - 1; i >= 0; --i) {
                    if (i == 0)
                        struct_index_hierarchy_string += " offset:";
                    else
                        struct_index_hierarchy_string += " member:";

                    struct_index_hierarchy_string += std::to_string(struct_index_hierarchy[i]);
                }
                ...
                if (ret == 1) {
                    ...
                    skip |= LogError(objlist, kVUID_Core_Shader_PushConstantOutOfRange, "%s in %s of %s is not updated.",
                                     struct_index_hierarchy_string.c_str(), string_VkShaderStageFlags(pStage->stage).c_str(),
    ...
    if (!found_stage) {
        ...
        skip |= LogError(objlist, kVUID_Core_Shader_PushConstantOutOfRange,
                         "Push constant is used in %s of %s. But %s of push constant range is not set in %s.",

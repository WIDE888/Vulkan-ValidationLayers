...
TEST_F(VkLayerTest, DrawWithoutUpdatePushConstants) {
    ...
    // push constant range: 0 - 99, used: 44 - 99
    char const *const vsSource =
        ...
        "} constants;\n"
        "void func1( float f ){\n"
        "   vec4 v4 = constants.v2[1];\n"
        "}\n"
        "void main(){\n"
        "   func1( constants.v2[0].w);\n"
        "   vec3 v3 = constants.v2[0].yzw;\n"
        "   for(int i=0;i<1;++i) {\n"
        "      v3 = constants.m[i]; \n"
        "   }\n"
        "}\n";

    // push constant range: 0 - 95, used: 32 - overlap with vsSoruce
    ...
    m_errorMonitor->SetDesiredFailureMsg(VK_DEBUG_REPORT_ERROR_BIT_EXT, "UNASSIGNED-CoreValidation-Shader-PushConstantOutOfRange");
    m_errorMonitor->SetDesiredFailureMsg(VK_DEBUG_REPORT_ERROR_BIT_EXT, "UNASSIGNED-CoreValidation-Shader-PushConstantOutOfRange");
